...
class GitHubPublisher(
  ...
  def publishAll(artifacts: (Seq[(os.Path, String)], Artifact)*): Unit = {
    log.info("OwO Publishing artifacts: " + artifacts)
  ...
  private def reportPublishResults(
    ...
    if (publishResults.forall(_.is2xx)) {
      log.info(s"Published $artifact to GitHub UwU")
    } else {
      val errors = publishResults.filterNot(_.is2xx).map { response =>
        s"Code: ${response.statusCode}, message: ${response.text()} Owo"
      }
      // Or just log? Fail later?
      throw new RuntimeException(
        s"Failed to publish $artifact to GitHub. Errors: \n${errors.mkString("\n")} UwU"
      )
    }
  }
}
